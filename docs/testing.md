...
| Discovery | `tests/**/*.test.ts` — 27 files |
| Total tests | 706, all passing |
...
| `answer-flow.test.ts` | 35 | Curated bot answers: an EXTERNAL or BOTH audience is refused above PUBLIC from every angle, an INTERNAL one may carry any classification, `validateAnswerDraft` agrees with `audienceReachesExternalZone`; question/answer length bounds; the training-phrase cap and minimum length; malformed and inverted effective dates; ARCHIVED is terminal; the FTS search text folds the question together with every phrasing; the verified-account gate defaults on for internal answers, is always off for an external audience, and may only be dropped for PUBLIC text; Telegram command validation (format, reserved built-ins, a required menu description) and `isAdvertisableCommand`, which lists only ACTIVE PUBLIC commands because a bot menu is visible before anyone verifies (§9, §23, §30) | `packages/domain/src/answer-flow.ts` |
| `cv-matching.test.ts` | 17 | Deterministic CV↔job matching: filler words are not evidence; technology names survive punctuation; a requirement needs a majority of its terms so one incidental word is not a match; an unmatched requirement never carries evidence; must-haves outweigh nice-to-haves; empty text and a requirement-less job score 0 without dividing by zero; contact details, links and the highest stated experience are read literally; an implausible year count is discarded; **no protected characteristic is ever extracted even when the CV states one**; the experience minimum stays null rather than guessing; and injected instructions in a CV match nothing (§26, §38) | `packages/domain/src/cv-matching.ts` |
| `misc-units.test.ts` | 50 | Validation combinators; date arithmetic and effective-date windows; password hashing, verification, rehash detection and policy; constant-time comparison; log redaction; FTS/LIKE escaping and tokenising; application-stage and leave-status machines; document chunking; text extraction; `parseApplication` (pipes, newlines or plain spaces, any order, `Name:`-style labels, a lower-case job code normalised, and a refusal rather than a guess when a field is missing); the in-memory rate limiter | `@corpus/shared`, `@corpus/domain`, `@corpus/auth`, `@corpus/knowledge`, `@corpus/security` |
...
| `bot-training.test.ts` | 56 | Dashboard-authored bot answers, which are served verbatim with no model turn: an INTERNAL-audience answer never reaches the external zone (FTS *and* LIKE fallback); an EXTERNAL-only answer never reaches the internal zone; a classification outside the caller's ceiling is excluded; an empty authorised set returns nothing; DRAFT and ARCHIVED answers are never served; effective dates open and close serving; the schema CHECK rejects a non-PUBLIC external answer even on a direct insert; retrieval never crosses a tenant; EMPLOYEE and MANAGER cannot author, list or read the backlog while HR, HR_ADMIN and SYSTEM_ADMIN can; the API refuses an EXTERNAL answer above PUBLIC; an archived answer cannot be revived; an EXTERNAL preview stays capped at PUBLIC even for SYSTEM_ADMIN; and a RESTRICTED-risk intent still reaches the gate and leaves an audited DENY rather than being short-circuited by a matching curated answer (§8, §9, §24) |
| `candidate-cvs.test.ts` | 66 | CVs are CONFIDENTIAL: EMPLOYEE and MANAGER are refused the listing, the body, the download and the match report while HR, HR_ADMIN and SYSTEM_ADMIN are allowed; an unauthenticated caller gets 401 everywhere; a CV never crosses a tenant; the intake refuses a non-CV format and anything over the 5 MB cap; a real DOCX is parsed to text while a PDF is stored unparsed with a paste-the-text warning and then matches once the text is entered by hand; and a CV carrying prompt injection is accepted, flagged, and raises a `DOCUMENT_INJECTION` event whose detail excludes the CV — while changing no application stage and satisfying no job requirement. Formats: **PDF, DOC and DOCX only** — TXT, Markdown, CSV, images and executables are all refused; a DOCX is read, while a PDF and a legacy `.doc` are stored unparsed with a paste-the-text warning and match once the text is entered. Intake is bot-only: there is no upload route, and the dashboard filters by source, text status and flag with facet counts describing the whole set rather than the filtered page. `/cv` is reserved so no curated answer can take it, and it appears in both command menus. On the **recruitment bot** the sender's own CV attaches with no staff attribution, a file with no command still works, `/cv` alone explains, and a Telegram id that never applied stores nothing. On the **employee bot** forwarding needs `candidate.document.manage` — EMPLOYEE and MANAGER are refused whatever the caption says — plus a caption naming an existing candidate by e-mail or application reference; an unknown address never creates a candidate, and a file with no caption is refused. The happy paths run through a stubbed `api.telegram.org`, so the whole download → extract → store → match chain is exercised. The full candidate journey is covered end to end — an unclassifiable message gets a recruitment reply rather than "contact HR", `/apply` in one line creates the candidate with the name **exactly as given**, links the Telegram id and returns the reference, and `/cv` then attaches the file. The guided flow is covered too: three answers collected one at a time, each classified by shape so order does not matter, the one-line form still accepted, `/cancel` abandoning a draft, ordinary conversation never captured, a half-finished draft holding no candidate record, and a photo answered rather than silently dropped (§8, §9, §12, §26) |
